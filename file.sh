  ...
  if [ -e "$repository" ]; then
    for file in $files; do
      ...
      if touch "$repository/$file"; then
        ...
        mkdir -p "./versions"
        mkdir -p "./versions/${files}_version"
        touch "./versions/${files}_version/v1_${files}"
        touch "./versions/${files}_version/version_manager.txt"
        echo "1" > "./versions/${files}_version/version_manager.txt" 
  ...
  if [ -e "$repository" ]; then
    for file in $files; do
      ...
      if [ -w "$repository/$file" ] && [ -r "$repository/$file" ]; then
        chmod u-rw "$repository/$file"
        versionFilePath="./versions/${file}_version/version_manager.txt"
        first_line=$(head -n 1 "$versionFilePath")
        new_number=$((first_line + 1))
        echo "$new_number" > "${versionFilePath}"
        touch "./versions/${file}_version/v${new_number}_file"
        sudo cat "$repository/$file" > "./versions/${file}_version/v${new_number}_file" 
...
rollback () {
  ...
  echo "$fileName"
  echo "$folderName"
  versionsPath="./versions/$1_version"
